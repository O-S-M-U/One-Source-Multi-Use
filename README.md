...
한국어 수익형 블로그 운영을 자동화하는 5단 파이프라인. 키워드 발굴 → 청사진 + 사실(facts) 수집 → HTML 생성 → 자기잠식·표절 검사 → 발행까지를 한 줄기로 묶는다. 단독·팀 운영 모두 지원하며, API 키 0개 환경에서도 룰 폴백으로 끝까지 돌아간다.

> 본 저장소는 v9 spec 기반으로 단계별 리팩터링이 진행 중이다. 현재 위치는 “collector Phase 1·2 완료, contents_maker 단순화 직전”.
...
| 단계 | 모듈 | 산출물 | 상태 |
|---|---|---|---|
| 0 | `interpreter.interpret` | KeywordContext (domain · intent · topic_summary) | ✅ 완료 |
| 1 | `keyword_researcher` | golden keyword + pool 관리 | ✅ 완료 |
| 2-1 | `Collector.phase1` | title · target_reader · paragraph_blueprint | ✅ 완료 |
| 2-1.5 | `embedder` | summary_embedding (jhgan/ko-sroberta-multitask) | ✅ 완료 |
| 2-1.6 | `blueprint.commercial_elements` | 추천 · 비교 · CTA 후보 | ✅ 완료 |
| 2-2 | `phase2.Phase2Collector` | normalized_sources (단락별 facts) | ✅ 완료 |
| 3 | `contents_maker` | refined_post (HTML) | ⚠️ 기존 Writer (재설계 예정) |
| 3-DB | SQLite 5 테이블 영속화 | keywords / contents / accounts ... | 🔜 다음 단계 |
| 4 | `checker` | 자기잠식·표절·구조 검증 | 🔜 |
| 5 | `publisher` | 티스토리 Playwright 자동 발행 | 🔜 |
