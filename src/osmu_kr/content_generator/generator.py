...
class Generator:
    ...
    def generate(self, keyword: Union[str, KeywordContext], *, save: bool = True,
        ...
        if save:
            record_id = self._save_record(
                ...
                blueprint=blueprint, phase2_result=phase2_result,
            )
    ...
    def _save_record(self, keyword, html, raw, images, *, status,
                     error_log, writer_used, title_final="",
                     blueprint=None, phase2_result=None) -> str:
        # v9 풍부 필드 — 모두 JSON 직렬화 텍스트로 저장 (csv/xlsx/sheets/sqlite 호환)
        target_reader_json = ""
        paragraph_blueprint_json = ""
        commercial_elements_json = ""
        summary_embedding_json = ""
        bp_title = ""
        if blueprint is not None:
            try:
                target_reader_json = json.dumps(
                    blueprint.target_reader.to_dict(), ensure_ascii=False,
                )
                paragraph_blueprint_json = json.dumps(
                    blueprint.paragraph_blueprint_json(), ensure_ascii=False,
                )
                commercial_elements_json = json.dumps(
                    blueprint.commercial_elements.to_dict(), ensure_ascii=False,
                )
                if blueprint.summary_embedding:
                    summary_embedding_json = json.dumps(
                        blueprint.summary_embedding, ensure_ascii=False,
                    )
                bp_title = blueprint.title
            except Exception as e:
                log.warning("[generator] blueprint 직렬화 실패: %s", e)
        normalized_sources_json = ""
        if phase2_result is not None:
            try:
                normalized_sources_json = json.dumps(
                    phase2_result.to_dict().get("sources_by_section", {}),
                    ensure_ascii=False,
                )
            except Exception as e:
                log.warning("[generator] phase2 직렬화 실패: %s", e)
        ...
        rec = ContentRecord(
            ...
            target_reader_json=target_reader_json,
            paragraph_blueprint_json=paragraph_blueprint_json,
            normalized_sources_json=normalized_sources_json,
            summary_embedding_json=summary_embedding_json,
            commercial_elements_json=commercial_elements_json,
            error_log=error_log,
